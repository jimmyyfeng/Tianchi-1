...
def pcsv(file_name,out_file=None):
    ...
    sum_=0
    for id in ids:
        # ct文件中概率最大的结节的概率再给它大点，0.6->0.996
        ...
        if probability>0.2:   f.iloc[max_index,4] = probability*0.01+0.99
        probability = f.iloc[max_index,4]
              
        # 第二大的概率0.6->0.96
        probability2 = f.iloc[max_index+1,4]
        if probability2>0.5:f.iloc[max_index+1,4]=0.1*probability2+0.9

...
center_list=glob('/mnt/7/0821_caffe_64_80/*_center.npy')
num_nodule=0
real_num=0
flag=0
f=open("/home/x/dcsb/Tianchi_pytorch/csv/center___.csv", "wa")
csv_writer = csv.writer(f, dialect="excel")
csv_writer.writerow(['seriesuid', 'coordX', 'coordY', 'coordZ','isnodule'])
for file in tqdm(center_list[::-1]):
    #import ipdb;ipdb.set_trace()
    file_name=file.split('/')[-1].split("_")[0]
    real_center=select(file_name)
    print real_center
    real_num +=real_center.shape[0]
    aa=np.load(file)
    if aa.shape[0]==0:
        continue
    ...
    for j in range(0,length):
        ...
        for i_,nodule in real_center.iterrows():   
            ...
            if distance<radius:
                print "I,m a real nodule",nodule['diameter_mm']

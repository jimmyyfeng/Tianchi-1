...
def load_ct_info(file_name):
    ...
    df_info=pd.read_csv(opt.mhd_information)
...
def world_2_voxel(arr,file_name):
    df_node=pd.read_csv(opt.mhd_information)
...
def select(file):
    ...
    '''
    df=pd.read_csv(opt.all_annotation)
...
def augument(imgs,mask=None,type=None):
    '''
    @imgs：原始待增强数据
...
def normalize(image, MIN_BOUND=-1000.0, MAX_BOUND=400.0):
    ...
    image[image < 0] = 0.
    return image    
def crop(image,image_mask,v_center=None,crop_size=[64,64,64]):
    '''
    @image：原始图像数据
    ...
    cubic_img=np.zeros(crop_size,dtype=np.float32)
    cubic_mask=np.zeros(crop_size,dtype=np.float32)
    if v_center is None:
        z,x,y=image.shape
        center_z=np.random.randint(half[0]+1,z-half[0],1)
        center_xy=np.random.randint(half[1]+1,x-half[1],2)
        v_center=np.concatenate([center_z,center_xy])
    v_center=check_center(crop_size,v_center,image.shape)
    cubic_img[:,:,:]=image[int(v_center[0]-half[0]):int(v_center[0]+half[0]),\
                int(v_center[1]-half[1]):int(v_center[1]+half[1]),\
                int(v_center[2]-half[2]):int(v_center[2]+half[2])]
    cubic_mask[:,:,:]=image_mask[int(v_center[0]-half[0]):int(v_center[0]+half[0]),\
                int(v_center[1]-half[1]):int(v_center[1]+half[1]),\
                int(v_center[2]-half[2]):int(v_center[2]+half[2])]
...
def make_mask(file_name):
    ...
    df_node= pd.read_csv(opt.all_annotation)
    name=file_name.split('/')[-1][:-4]
